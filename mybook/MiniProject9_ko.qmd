...
1.  가장 기본적인 예. reprex를 실행하려면 Ctrl + C를 사용하여 아래 코드를 모두 복사한 다음 콘솔 창에서 reprex()를 실행하십시오.
...
2.  이제 미니 프로젝트의 첫 번째 라운드에서 변수 SEX를 재코딩하는 데 사용한 일부 코드를 가져와 해당 reprex를 살펴보겠습니다. 다시 말하지만, 다음 청크의 모든 코드를 복사한 다음 화면 하단의 콘솔에 reprex()를 입력하십시오.
...
4.  이제 약간 더 복잡한 예를 살펴보겠습니다. 이것은 원래 미니 프로젝트 6에서 사용된 일부 코드입니다. 이 청크의 모든 코드를 복사한 다음 콘솔에서 reprex()를 실행하십시오. 참고: 데이터가 로드되는 데 시간이 걸릴 수 있습니다.
...
하지만... 이 마지막 두 예는 ***최소한으로 재현 가능한 예제***가 아닙니다. 이를 다시 실행하려면 도움을 주는 사람이 데이터베이스 및/또는 참조하는 데이터셋에 액세스할 수 있어야 합니다. 그리고 해당 데이터셋이 크거나 읽는 데 시간이 걸리는 경우, 실제로 묻는 것이 플롯 코드에 관한 것이라면 문제 해결 프로세스에 오버헤드가 됩니다. 그렇다면 이 문제 설명을 더 작고 "최소한"으로 만들려면 어떻게 해야 할까요?
...
플롯에 대해 묻는 reprex를 만드는 경우 코드가 이전에 3단계의 데이터 조작이 있는 경우 데이터 조작을 복제하지 마십시오. 플롯에 들어가는 데이터의 형식을 설명하는 `tribble`로 바로 이동하십시오. 그리고 해당 데이터 조작의 출력이 무엇인지 추측하고 "이상화된" tribble을 지정하지 마십시오. 조작 후 가지고 있는 데이터를 검사하고 조작된 데이터에서 보는 것과 똑같이 플롯에 필요한 열만 전달하십시오. 원하는 경우 연속 결과에 대해 임의의 값을 사용할 수 있습니다. 요점은 반드시 ***값***이 무엇인지가 아니라 데이터의 구조와 관련이 있는 경우가 많습니다.
...
-   rm(list = ls())로 청크를 시작하지 마십시오. 도움을 주려는 사람의 현재 환경을 지워버릴 것입니다. 그들은 아마 감사하지 않을 것입니다. (작업 공간은 SAS 작업 디렉토리와 같으며 이 명령은 SAS kill을 실행하는 것과 같습니다.)
...
더 많은 해야 할 일과 하지 말아야 할 일: <https://reprex.tidyverse.org/articles/reprex-dos-and-donts.html>
...
다른 사람이 문제를 이해하고 해결하는 데 도움이 될 더 최소한의 reprex를 만드십시오.
