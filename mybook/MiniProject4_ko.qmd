...
![인구 통계 표](img/MiniProject4_demog_table.png). 챌린지를 완료한 후 업데이트된 테이블은 다음과 유사해야 합니다:![최종 인구 통계 표](img/MiniProject3_demog_summary_table.png)
...
2.  Big N 및 small n 개수 계산 및 데이터프레임 조인 (프로젝트 2)
...
3.  연령 그룹에 대한 개수 계산 (챌린지 1) 연령 그룹별 개수를 계산하고 프로젝트 2에서 생성된 성별 개수와 함께 병합합니다.
...
아래 `myData`에서 `age`로 정렬하면 R은 연령 범주 ">=65"를 먼저 배치합니다. 이는 R이 문자 변수를 영숫자 순으로 정렬하기 때문이므로 ">"가 "1"보다 먼저 옵니다.
...
8.  프로젝트 4 . 이제 위에서 만든 두 개의 출력 데이터프레임을 결합합니다. `bind_rows` 함수를 사용하여 두 개의 데이터프레임(쉼표로 구분)을 함께 조인합니다. Bind_rows는 SAS의 SET 문과 매우 유사하며 여러 데이터프레임, 목록 또는 데이터프레임 목록을 하나로 바인딩하는 데 사용됩니다.
