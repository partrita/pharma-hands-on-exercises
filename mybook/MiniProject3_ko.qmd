...
이 프로젝트에서는 다음과 같은 인구 통계 요약 테이블을 만드는 것을 목표로 합니다: ![인구 통계 표](img/MiniProject3_demog_table.png).
...
3.  인구 통계 테이블에 필요한 연령 평균(치료 및 성별별로 그룹화)을 담을 데이터프레임(age_mean)을 만듭니다.
...
`pivot_longer()` 함수는 데이터를 "넓은" 형식에서 "긴" 형식으로 피벗(또는 전치)합니다. 이 함수에는 세 가지 주요 인수가 있습니다. `cols`는 R에게 피벗(전치)할 열을 알려주고, `names_to`는 R에게 피벗된 변수의 ***이름***을 담을 새 변수의 이름을 알려주고, `values_to`는 R에게 피벗된 변수의 ***값***을 담을 새 변수의 이름을 알려줍니다.

`pivot_longer` 전에 `select()` 함수를 사용하면 피벗된 전치 데이터에 표시하려는 변수만 선택할 수 있습니다. 대부분의 경우 모든 변수를 전치하고 싶지는 않을 것입니다. 피벗(전치)할 열을 세 가지 방법으로 정의할 수 있습니다. 명시적으로 이름을 지정하거나, 열 또는 변수 번호로 식별하거나, `-<VARIABLE>`을 사용하여 R에게 피벗에서 ***제외***할 변수를 알려줄 수 있습니다. 둘 이상의 변수를 제외하려면 `c()` 컬렉션 함수를 사용하여 변수 이름 벡터를 정의합니다. 아래 코드에서 위에서 계산한 요약 통계 목록을 select 함수에 추가하십시오(최소값과 최대값은 제외하고 범위는 포함).
