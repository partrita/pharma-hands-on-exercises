이 연습에서는 다섯 가지 {ggplot2} 확장 패키지를 소개합니다:
...
"ggplot2를 사용한 데이터 시각화" 치트 시트를 보고 상자 그림에 대한
...
`{ggridges}` 패키지는 이러한 분포 표현을 사용하여 공간을 최소화하기 위해
